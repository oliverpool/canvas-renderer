...
// RenderPreview is adapted from https://github.com/tdewolff/canvas/blob/master/examples/preview/main.go
...
func RenderPreview(w canvas.Writer) error {
	...
	return c.WriteFile("preview.pdf", w)
}
...
func draw(c *canvas.Context, fontFamily *canvas.FontFamily) {
	...
	lenna, err := os.Open(filepath.Join(testDir, "lenna.png"))
	if err != nil {
		panic(err)
	}
	img, err := NewPNGImage(lenna)
	if err != nil {
		panic(err)
	}
	c.Rotate(5)
